  ...
  return (
    <AppScreen>
      ...
      <View style={styles.brandRow}>
        <View style={[styles.brandMark, { backgroundColor: palette.primary }]}><Text style={styles.brandMarkText}>↔</Text></View>
        ...
        <Pressable onPress={() => router.push('/settings')} style={[styles.levelBadge, { backgroundColor: palette.panel, borderColor: palette.border }]} accessibilityLabel={`Level ${level}. Open settings.`}>
      ...
      <Pressable
        ...
        style={({ pressed }) => [styles.daily, { backgroundColor: palette.secondary }, pressed && styles.pressed]}
        accessibilityRole="button"
      >
        <View style={styles.dailyTop}>
          <Text style={styles.dailyKicker}>{todayResult ? 'REPLAY AVAILABLE' : 'TODAY’S CHALLENGE'}</Text>
          <Text style={styles.dailyStreak}>🔥 {data.daily.currentStreak} day streak</Text>
        </View>
        <Text style={styles.dailyTitle}>Daily Switch</Text>
        <Text style={styles.dailyDescription}>{todayResult ? `Official score ${todayResult.score.toLocaleString()} · Replays won’t replace it.` : MODE_CONFIG.daily.description}</Text>
        <View style={styles.dailyAction}><Text style={styles.dailyActionText}>{todayResult ? 'Replay today' : 'Play today'}</Text><Text style={styles.dailyArrow}>→</Text></View>
      </Pressable>
...
function ModeCard({ icon, title, copy, accent, onPress, wide }: { icon: string; title: string; copy: string; accent: string; onPress(): void; wide: boolean }) {
  const palette = usePalette();
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.modeCard, wide && styles.modeCardWide, { backgroundColor: palette.panel, borderColor: palette.border }, pressed && styles.pressed]}>
      <View style={[styles.modeIcon, { backgroundColor: accent }]}><Text style={styles.modeIconText}>{icon}</Text></View>
...
const styles = StyleSheet.create({
  ...
  brandMarkText: { color: '#071510', fontSize: 29, fontWeight: '900' },
  brandCopy: { flex: 1 },
  ...
  dailyTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 },
  ...
  modeIconText: { color: '#101526', fontSize: 24, fontWeight: '900' },
  modeCopy: { flex: 1 },
